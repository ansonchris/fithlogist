...
class FirthLogisticRegression(BaseEstimator, ClassifierMixin):
    ...
    Renames "intercept" to "const" (statistical convention) and places const first in summaries.
    ...
    fit_const : bool, default=True  # Renamed from fit_intercept
        Whether to include a constant term (const) in the model.
    ...
    Attributes
    ----------
    bse_ : ndarray of shape (n_features + [1 if fit_const else 0],)
        Standard errors of the coefficients (including const if fitted).
    ...
    const_ : float  # Renamed from intercept_
        Firth-corrected constant term (0 if fit_const=False).
    loglik_ : float
    ...
    def __init__(
        ...
        pl_max_stepsize=5,
        tol=0.0001,
        fit_const=True,  # Renamed: fit_intercept → fit_const
        skip_pvals=False,
        skip_ci=False,
        alpha=0.05,
        wald=False,
        ...
        self.tol = tol
        self.fit_const = fit_const  # Renamed: fit_intercept → fit_const
    ...
    def fit(self, X, y):
        ...
        # Add constant column if enabled (append column of 1s to X)
        if self.fit_const:  # Renamed: fit_intercept → fit_const
        ...
        # Split constant term from coefficients (for scikit-learn compatibility)
        if self.fit_const:  # Renamed: fit_intercept → fit_const
            self.const_ = self.coef_[-1]  # Renamed: intercept_ → const_
            self.coef_ = self.coef_[:-1]
        else:
            self.const_ = 0.0  # Renamed: intercept_ → const_
    ...
    def summary(self, xname=None):
        ...
        # --------------------------
        # Critical Change: CONST FIRST in summary
        # --------------------------
    ...
    def decision_function(self, X):
        """
        Compute linear decision function (z = X @ coef + const).
        ...
        # Compute linear scores (const instead of intercept)
        scores = X @ self.coef_ + self.const_  # Renamed: intercept_ → const_
        return scores
...
def _firth_newton_raphson(X, y, max_iter, max_stepsize, max_halfstep, tol, mask=None):
    ...
    X : ndarray of shape (n_samples, n_params)
        Feature matrix (including const if fitted).
    y : ndarray of shape (n_samples,)
...
if __name__ == "__main__":
    ...
    # Fit Firth logistic regression model (with const)
    model = FirthLogisticRegression(
        fit_const=True,  # Include constant term (const)
    ...
    # Print model summary (CONST as FIRST row)
